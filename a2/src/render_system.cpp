									...
									const mat3 &projection)
{
	...
	else if (render_request.used_effect == EFFECT_ASSET_ID::CHICKEN || render_request.used_effect == EFFECT_ASSET_ID::EGG)
	{
		...
		if (render_request.used_effect == EFFECT_ASSET_ID::CHICKEN)
		{
			...
			//const float color[] = { 1.0,1.0,0 };
			//registry.colors.get(entity).x = 1.0; 
				// ADDING CODE FOR A1 3a
			//GLint color_uloc = glGetUniformLocation(program, "fcolor");
			//const vec3 color = registry.colors.has(entity) ? registry.colors.get(entity) : vec3(1);
			//float color[] = { 11.f,0.f,0.f };
			bool alive = registry.players.get(entity).is_alive;
			//float counter = registry.lightup.get(entity).counter_ms;
			bool has_eaten = registry.players.get(entity).has_eaten;
			//registry.lightup.emplace(entity);
			//int lightup = registry.lightup.get(entity).counter_ms;
			const GLint eat = 1.0; 
			const GLint not_eat = 0;
 
			//float color[] = { 1.0,1.0,1.0,1.0};
			//glUniform4fv(light_up_uloc, 1, color); 
			//glUniform4fv(light_up_uloc, 1, &not_eat); 
			if (alive && has_eaten) {
				glUniform1iv(light_up_uloc, 1, &eat);
			}
			else if (alive && !has_eaten) {
				glUniform1iv(light_up_uloc, 1, &not_eat);
			}
			...
			// chicken is dead change to diff color
			
			/*if (!alive) {
				float dead[] = { 0.f,0.f,0.f };
				//registry.colors.get(entity).x = 1.0;
				//registry.colors.get(entity).y = 0;
				//registry.colors.get(entity).y = 0;
				glUniform3fv(color_uloc, 1, (float *)&dead);
